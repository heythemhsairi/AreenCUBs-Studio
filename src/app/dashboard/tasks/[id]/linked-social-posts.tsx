  ...
  return (
    <div className="glass rounded-2xl p-5">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-content-3">
      ...
      {posts.length === 0 ? (
        <p className="text-xs text-content-3">Aucun post lié à cette tâche.</p>
      ) : (
        <div className="space-y-1.5">
          {posts.map((p) => (
            <div
              key={p.id}
              className="flex items-center gap-2 rounded-lg bg-surface/4 px-3 py-2"
            >
              <span className="shrink-0 text-sm">{platformIcons(p.platforms)}</span>
              <span className="flex-1 truncate text-sm text-content-2">{p.title}</span>
              <span className={cn(
                "shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium",
                STATUS_COLORS[p.status] ?? "bg-ink/10 text-content-3",
              )}>
