    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-content">{c.reportsTitle}</h1>
        </div>
        <select
          value={clientFilter}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {[
          ...
          <div key={label} className="rounded-xl border border-line bg-surface p-4 flex flex-col gap-2">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-line bg-surface p-4">
          ...
          {platformCounts.size === 0 ? (
            <p className="text-xs text-content-3">—</p>
          ) : (
                ...
                .map(([platform, count]) => {
                  ...
                  return (
                    <div key={platform} className="flex items-center gap-2">
                      <span className="w-20 text-xs text-content-2 capitalize shrink-0">{platform}</span>
                      ...
                      <span className="w-6 text-right text-xs text-content-3">{count}</span>
                    </div>
                  );
                })}
            </div>
                ...
                .map(([type, count]) => {
                  ...
                  return (
                    <div key={type} className="flex items-center gap-2">
                      <span className="w-20 text-xs text-content-2 shrink-0">
      ...
      <div className="rounded-xl border border-line bg-surface">
        ...
        {filteredPlans.length === 0 ? (
          <div className="py-10 text-center text-sm text-content-3">
            {c.noPlansFound}
          </div>
        ) : (
                  ...
                  return (
                    <tr key={plan.id} className="hover:bg-surface-2 transition-colors">
                      ...
                      <td className="px-4 py-3 text-content-2">
                        ...
                        {plan.theme && <span className="text-content-3 ml-1">— {plan.theme}</span>}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right text-content-2">{total}</td>
                      <td className="px-4 py-3 text-right text-content-2">{approved}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <span className="text-xs text-content-3 w-7 text-right">{pct}%</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <Link
                          href={`/dashboard/content/plans/${plan.id}`}
                          className="flex items-center gap-1 text-content-3 hover:text-accent2 transition-colors"
                        >
